...
struct kionix_accel_driver {
	...
	int accel_data[3];
	int accel_cali[3];
	char calibrate_buf[KIONIX_CAL_BUF_SIZE];
	u8 axis_map_x;
	u8 axis_map_y;
	u8 axis_map_z;
	bool negate_x;
...
static int accel_poll_thread(void *data)
{
	...
	while (true) {
		...
		if (acceld->accel_delay_change) {
			if (atomic_read(&acceld->delay) <= 10)
				set_wake_up_idle(true);
			else
				set_wake_up_idle(false);
			acceld->accel_delay_change = false;
		}
...
static int kionix_accel_enable(struct kionix_accel_driver *acceld)
{
	...
	if (err < 0) {
		KMSGERR(&acceld->client->dev,
			...
			__func__, err);
		printk(KERN_ERR"%s: kionix_accel_operate returned err = %d\n",__func__,err);
		goto exit;
	}
	...
	if (enable) {
		if (pre_enable == 0) {
			...
			if (acceld->accel_drdy==0) {
				ktime = ktime_set(0,
					atomic_read(&acceld->delay) * NSEC_PER_MSEC);
				hrtimer_start(&acceld->timer_acc, ktime, HRTIMER_MODE_REL);
			}
		...
		if (pre_enable == 1) {
			if (acceld->accel_drdy==0) {
				hrtimer_cancel(&acceld->timer_acc);
			}
					...
					unsigned int delay_ms)
{
	...
	acceld->accel_delay_change = true;
	return 0;
}

#ifdef CONFIG_HY_DRV_ASSIST
static ssize_t kionix_accel_ic_show(struct device *dev,
                                 struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%s\n",acc_ic);
}
static ssize_t kionix_accel_vendor_show(struct device *dev,
                                 struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%s\n","KIONIX");
}
static ssize_t kionix_accel_exist_show(struct device *dev,
                                     struct device_attribute *attr, char *buf)
{
    return sprintf(buf, "%d\n", acc_sensor);
}
...
static int kionix_do_calibration(struct sensors_classdev *sensors_cdev,
		int axis, int apply_now)
{
	...
	int xyz[3] = { 0 };
		...
		int err;
	err = acceld->kionix_accel_update_odr(acceld, 20); //set ODR to 50Hz/20ms
	if (err < 0)
	{
		KMSGERR(&acceld->client->dev,
			"%s: set ODR error, err = %d. Abort.\n",
			__func__, err);
		return err;
	}

	err = kionix_accel_cdev_enable(sensors_cdev, 1); //enable sensor
	if (err < 0)
	{
		KMSGERR(&acceld->client->dev,
			"%s: sensor enable error, err = %d. Abort.\n",
			__func__, err);
		return err;
	}
	...
	acceld->accel_cali[0] = acceld->accel_cali[1] = acceld->accel_cali[2] = 0;
	sum_x = sum_y = sum_z = 0;
	for(i = 0; i<CAL_COUNT; i++) //Get multi-group data for average
	{
		acceld->kionix_accel_report_accel_data(acceld);
		sum_x  += acceld->accel_data[acceld->axis_map_x];
		sum_y  += acceld->accel_data[acceld->axis_map_y];
		sum_z  += acceld->accel_data[acceld->axis_map_z];
		printk("axis=%d, acceld->accel_data[acceld->axis_map_x]=%d, acceld->accel_data[acceld->axis_map_y]=%d, acceld->accel_data[acceld->axis_map_z]=%d ---CYY\n", axis,
	...
	sum_z = sum_z/CAL_COUNT;
      printk("sum_x=%d, sum_y=%d, sum_z=%d ---CYY\n",
				sum_x, sum_y, sum_z);
	//X, Y, Z offset
	acceld->accel_cali[acceld->axis_map_x] = -sum_x;
	acceld->accel_cali[acceld->axis_map_y] = -sum_y;
	acceld->accel_cali[acceld->axis_map_z] = -sum_z + 1024;
      printk("acceld->accel_cali[acceld->axis_map_x] =%d, acceld->accel_cali[acceld->axis_map_y]=%d, acceld->accel_cali[acceld->axis_map_z]=%d ---CYY\n",
				acceld->accel_cali[acceld->axis_map_x] , acceld->accel_cali[acceld->axis_map_y], acceld->accel_cali[acceld->axis_map_z]);
	switch (axis) {
	case AXIS_X:
		xyz[0] = acceld->accel_cali[acceld->axis_map_x];
		xyz[1] = 0;
		xyz[2] = 0;
		break;
	case AXIS_Y:
		xyz[0] = 0;
		xyz[1] = acceld->accel_cali[acceld->axis_map_y];
		xyz[2] = 0;
		break;
	case AXIS_Z:
		xyz[0] = 0;
		xyz[1] = 0;
		xyz[2] = acceld->accel_cali[acceld->axis_map_z];
		break;
	case AXIS_XYZ:
		xyz[0] = acceld->accel_cali[acceld->axis_map_x];
		xyz[1] = acceld->accel_cali[acceld->axis_map_y];
	...
	memset(acceld->calibrate_buf, 0, sizeof(acceld->calibrate_buf));
	snprintf(acceld->calibrate_buf, sizeof(acceld->calibrate_buf),
			"%d,%d,%d", xyz[0], xyz[1], xyz[2]);
	sensors_cdev->params = acceld->calibrate_buf;

	return 0;
}

static int kionix_write_cal_params(struct sensors_classdev *sensors_cdev,
		struct cal_result_t *cal_result)
{
	...
	acceld->accel_cali[acceld->axis_map_x] = cal_result->offset_x;
	acceld->accel_cali[acceld->axis_map_y] = cal_result->offset_y;
	acceld->accel_cali[acceld->axis_map_z] = cal_result->offset_z;

	KMSGERR(&acceld->client->dev,
			"read accel calibrate bias %d,%d,%d\n",
			acceld->accel_cali[acceld->axis_map_x], acceld->accel_cali[acceld->axis_map_y], acceld->accel_cali[acceld->axis_map_z]);

	return 0;
}
/*
 * kionix_acc_timer_handle() - timer_acc handle to schedule workerthread
 * @hrtimer: the hrtimer struct
 *
 * handle gets called based on the poll time to schedule worker thread
 */
static enum hrtimer_restart kionix_acc_timer_handle(struct hrtimer *hrtimer)
{
       ktime_t ktime;
       struct kionix_accel_driver *sensor;

       sensor = container_of(hrtimer, struct kionix_accel_driver, timer_acc);

       ktime = ktime_set(0,
                        atomic_read(&sensor->delay) * NSEC_PER_MSEC);
       hrtimer_forward_now(&sensor->timer_acc, ktime);
			      ...
			      const struct i2c_device_id *id)
{
	...
	kionix_accel_update_direction(acceld);
input_set_events_per_packet(acceld->input_dev, 100);
	acceld->accel_workqueue = create_freezable_workqueue("Kionix Accel Workqueue");
	INIT_WORK(&acceld->accel_work, kionix_accel_work);
	hrtimer_init(&acceld->timer_acc, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	acceld->timer_acc.function = kionix_acc_timer_handle;
	...
	acceld->cdev.sensors_calibrate = kionix_do_calibration;
	acceld->cdev.sensors_write_cal_params = kionix_write_cal_params;
...
#ifdef CONFIG_HY_DRV_ASSIST
	accel_assist_register_attr("ic",&kionix_accel_ic_show,NULL);
	accel_assist_register_attr("vendor",&kionix_accel_vendor_show,NULL);
	accel_assist_register_attr("exist",&kionix_accel_exist_show,NULL);
#endif
	acc_sensor=1;
	return 0;
